...
import in.zygertechnology.zygererp.service.PrintTemplateRenderer;
...
public class SalesController {
    ...
    private final PrintTemplateRenderer htmlPrinter;
    ...
    @Operation(summary = "Print a sales document as HTML (opens the browser print dialog)")
    @GetMapping("/{type}/{id}/print/html")
    ResponseEntity<String> printHtml(
            @Parameter(description = "Document type") @PathVariable String type,
            @Parameter(description = "Document ID") @PathVariable Long id) {
        Map<String, Object> row = svc.getRow(key(type), id);
        String docNo = String.valueOf(row.getOrDefault("docNo", type)).replaceAll("[^A-Za-z0-9_-]", "_");
        String html = switch (type) {
            case "sales-invoice" -> htmlPrinter.salesInvoice(row);
            case "sales-dc" -> htmlPrinter.deliveryChallan(row, type);
            default -> throw new IllegalArgumentException("No HTML print template for type: " + type);
        };
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "inline; filename=\"" + docNo + ".html\"")
